...
Address_check_for_permit.py allows you to paste or type in an address directly into the command line. This is useful for going through a list of new HVAC jobs and seeing from the address if the jobs require permits with a simple yes or no. The script reads Permit_fee_check.txt.

Customer_data.txt contains information about customer:
Customer name
Job address
Phone number
...
Each HVAC permit pdf has a corresponding script to fill it. They use Customer_data.txt, a signature.png (where relevent), and will prompt for printing and deleting the file. Amherst does not prompt for printing because it is done online. 